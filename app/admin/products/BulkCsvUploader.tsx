...
export default function BulkCsvUploader() {
  ...
  const onDragLeave = () => {
    dropRef.current?.classList.remove("ring-2", "ring-blue-500", "bg-blue-50/50");
  };
  ...
  const downloadTemplate = () => {
    const header = [
      "Item Type","Product ID","Product Name","Product Type","Product Code/SKU","SAGE Code","Brand Name",
      "Product Description","Product Weight","isCreated","XS","SM","MD","LG","XL","X2","X3","Category",
    ].join(",");
    const sample = [
      "Shirt","P001","Girls Polo","Top","SKU-GIRLS-POLO-01","SG-100","BrandX",
      "Cotton polo shirt","0.25","false","true","true","true","true","true","false","false","Schoolwear",
    ].join(",");
    const csv = header + "\n" + sample + "\n";
  ...
  const Stat = ({ label, value, tone = "default" }:{
    label: string; value: number | string; tone?: "success"|"warn"|"danger"|"default";
  }) => {
    ...
    <div className="w-full">
      ...
      <div className="mb-6 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold leading-tight">Add Products — Bulk CSV</h1>
            <p className="text-sm text-gray-600">Upload a CSV, validate, and insert in one go. Use the official template to avoid errors.</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={downloadTemplate} className="rounded-lg border border-gray-300 px-3 py-2 text-sm hover:bg-gray-50">
            Download Template
          </button>
          <button onClick={resetAll} className="rounded-lg border border-gray-300 px-3 py-2 text-sm hover:bg-gray-50">
            Reset
          </button>
          <button
          ...
          <div className="rounded-2xl border border-gray-200 bg-white shadow-sm">
            <div className="border-b border-gray-100 px-6 py-4">
              ...
              <p className="text-sm text-gray-600">
                Drag & drop a <span className="font-medium">.csv</span> here, or click to browse. Required columns:
                <span className="font-medium"> Product ID</span>, <span className="font-medium">Product Name</span>, <span className="font-medium">Product Code/SKU</span>.
              </p>
            </div>
            ...
            <div className="p-6">
              ...
              >
                ...
                <div className="flex h-12 w-12 items-center justify-center rounded-full bg-white shadow-sm">
                  <svg viewBox="0 0 24 24" className="h-7 w-7 text-gray-500" fill="none" stroke="currentColor" strokeWidth="1.6">
                    <path d="M12 16V4m0 0l-4 4m4-4l4 4" strokeLinecap="round" strokeLinejoin="round" />
                    <rect x="3" y="14" width="18" height="6" rx="2" className="stroke-gray-400" />
                  </svg>
                </div>
                <div className="text-sm text-gray-700">
                  <span className="font-medium">Drop your CSV here</span> or click to select
                </div>
          ...
          <div className="rounded-2xl border border-gray-200 bg-white shadow-sm">
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <Stat label="Total Rows" value={result?.totalRows ?? 0} />
                {"validated" in (result ?? {}) && <Stat label="Validated" value={(result as any).validated ?? 0} />}
                {"inserted" in (result ?? {}) && <Stat label="Inserted" value={(result as any).inserted ?? 0} tone="success" />}
                {"skipped" in (result ?? {}) && <Stat label="Skipped" value={(result as any).skipped ?? 0} tone="warn" />}
                {"failed" in (result ?? {}) && <Stat label="Failed" value={(result as any).failed ?? 0} tone="danger" />}
              </div>

              {errorCsvUrl && (
                <div className="rounded-xl bg-amber-50 px-3 py-2 text-sm text-amber-800 ring-1 ring-amber-200">
                  Some rows need fixes.{" "}
                  <a href={errorCsvUrl} target="_blank" rel="noreferrer" className="font-semibold underline">
      ...
      <div className="mt-6 rounded-2xl border border-gray-200 bg-white p-4 text-sm text-gray-600 shadow-sm">
        <div className="flex items-center gap-2">
          <svg viewBox="0 0 24 24" className="h-5 w-5 text-gray-500" fill="none" stroke="currentColor" strokeWidth="1.8">
            <path d="M12 9v3.5m0 3v.5" strokeLinecap="round"/>
            <circle cx="12" cy="12" r="9"></circle>
          </svg>
          <span>
            Tip: Ensure headers exactly match the template. Booleans accept <code>true/false</code>, <code>1/0</code>, or <code>yes/no</code>.
          </span>
        </div>
      </div>
    </div>
